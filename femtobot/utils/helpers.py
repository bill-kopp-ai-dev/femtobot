"""Utility functions for femtobot."""
...
_TOOL_RESULTS_DIR = ".femtobot/tool-results"
...
def build_status_content(
    *,
    ...
    lines = [
        f"\U0001f408 femtobot v{version}",
...
def sync_workspace_templates(workspace: Path, silent: bool = False) -> list[str]:
    ...
    try:
        tpl = pkg_files("femtobot") / "templates"
    except Exception:
        return []
    if not tpl.is_dir():
        return []
...
def load_bundled_template(template_name: str) -> str | None:
    """Read a bundled template file from the femtobot package."""
    from importlib.resources import files as pkg_files

    with suppress(Exception):
        tpl = pkg_files("femtobot") / "templates" / template_name
        if tpl.is_file():
            return tpl.read_text(encoding="utf-8")
    return None


def ensure_instance_structure(instance_dir: Path, workspace_dir: Path | None = None) -> list[Path]:
    """Ensure the basic instance directory structure exists.

    Creates:
    - instance_dir/history/
    - instance_dir/history/cli_history
    - instance_dir/workspace/
    - instance_dir/workspace/memory/
    - instance_dir/workspace/memory/history.jsonl

    Args:
        instance_dir: Root instance directory (.femtobot or .femtobot_<suffix>)
        workspace_dir: Optional custom workspace path. If None, uses instance_dir/workspace.
    ...
    # Create cli_history file
    cli_history = history_dir / "cli_history"
    if not cli_history.exists():
        cli_history.touch()
        created.append(cli_history)

    # Determine workspace path
    ws = workspace_dir if workspace_dir else instance_dir / "workspace"

    # Ensure workspace subdirectories
    memory_dir = ws / "memory"
    memory_dir.mkdir(parents=True, exist_ok=True)

    # Create memory history file
    memory_history = memory_dir / "history.jsonl"
    if not memory_history.exists():
        memory_history.touch()
        created.append(memory_history)

    # Create .gitignore in workspace if not exists
    gitignore = ws / ".gitignore"
    if not gitignore.exists():
        gitignore.write_text("""# Femtobot workspace
__pycache__/
*.pyc
.env
*.log
...
def create_instance_gitignore(instance_dir: Path) -> Path:
    """Create .gitignore in instance root.
...
Next steps:
  1. Edit {instance_dir / "config.json"} to add your API key
  2. Run: femtobot agent -m "Hello!"
"""
    if created_files:
        summary += "\nCreated files:\n"
