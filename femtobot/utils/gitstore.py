...
# Maximum number of characters we will emit from
# ``summarize_working_tree``'s diff section.  The full git diff is
# potentially unbounded; without a cap, a runaway Dream run could
# produce an enormous audit record that bloats git history.  The
# 6000-char cap mirrors nanobot v0.2.1 + #4188.
...
class GitStore:
    ...
    def init(self) -> bool:
        ...
        try:
            ...
            porcelain.commit(
                str(self._workspace),
                message=b"init: femtobot memory store",
                author=b"femtobot <femtobot@dream>",
                committer=b"femtobot <femtobot@dream>",
            )
    ...
    def _head_tree(repo) -> object | None:
        ...
        Mirrors nanobot's helper of the same name.  Used by
        :meth:`summarize_working_tree` to read the on-disk version of
        a tracked file from the previous commit.
        """
        try:
            head = repo.refs[b"HEAD"]
        except KeyError:
    ...
    def summarize_working_tree(self, paths: list[str]) -> str:
        ...
        This is the seam that ``dream_content_diff`` uses to gate cursor
        advance on real edits.  It is also a faithful audit record
        because it never reflects the LLM's self-report.
        ...
        Imported from nanobot (see nanobot/utils/gitstore.py:307) to
        close the Dream R2 regression documented in
                ...
                for path in paths:
                    ...
                    try:
                        ...
                        summary_lines.append(
                            f"{path}: binary or non-UTF-8 file changed"
                        )
                        continue
