"""WebSocket server channel: femtobot acts as a WebSocket server and serves connected clients.

IMPORTANT - DO NOT DELETE THIS FILE
====================================

This module is CRITICAL for the Femtobot architecture and MUST NOT be deleted.

Purpose:
---------
This WebSocket server channel enables Femtobot to act as a server that accepts
connections from clients. This is the foundation for:

1. **GUI Support**: Provides the WebSocket infrastructure for future GUI clients
   (web-based or desktop applications) to connect to Femtobot.

2. **A2A Protocol**: This channel is essential for the Agent-to-Agent protocol
   implementation, allowing different Femtobot instances to communicate over
   WebSocket connections in a Docker container network.

3. **Remote Clients**: Enables remote clients (e.g., a web frontend) to connect
   to a running Femtobot instance via WebSocket for real-time interaction.

Why It Was Previously Deleted (and restored):
----------------------------------------------
During initial refactoring, this file was mistakenly marked as "WebUI-only" and
removed. However, it is actually a CORE infrastructure component that supports:
- Real-time streaming responses
- Multi-client connections
- Session management per client
- Heartbeat/keepalive for long connections

The WebSocket channel is configured in .femtobot/config.json under:
  channels.websocket.enabled = true/false

Usage:
-------
When a GUI or A2A client is implemented, this module will handle:
- WebSocket handshake and authentication
- Message routing between clients and the agent loop
- Session management for each connected client
- Streaming of agent responses in real-time

For A2A Docker Setup:
--------------------
In a multi-container setup, one Femtobot instance (the Supervisor) can connect
to another Femtobot instance (the Worker) via WebSocket to delegate tasks.
This module provides that connectivity.

Dependencies:
-------------
- websockets (Python library)
- aiohttp (for HTTP health checks if needed)
- tornado (optional, for production WS servers)

Keep this file synchronized with any changes to the agent loop message handling.
"""
...
from femtobot.bus.events import OUTBOUND_META_AGENT_UI, OutboundMessage
from femtobot.bus.queue import MessageBus
from femtobot.channels.base import BaseChannel
...
from femtobot.session.goal_state import goal_state_ws_blob
from femtobot.session.webui_turns import websocket_turn_wall_started_at
from femtobot.utils.media_decode import (
    FileSizeExceeded,
    save_base64_data_url,
)


def websockets_server_logger():
...
def normalize_mcp_preset_mentions(x):
    return x
...
class DummyWorkspaces:
    def scope_for_new_chat(self, *args, **kwargs):
        return DummyScope()

    def scope_for_set_request(self, *args, **kwargs):
        return DummyScope()

    def scope_for_message(self, *args, **kwargs):
        return DummyScope()

    def persist_scope(self, *args, **kwargs):
        pass
...
class DummyMedia:
    ...
    def sign_or_stage_media_path(self, p):
        return None
...
class WebSocketConfig(Base):
    ...
    - ``token_issue_path``: If non-empty, **GET** (HTTP/1.1) to this path returns JSON
      ...
      femtobot and shares the asyncio loop, use a thread or async HTTP client for GET—do not call
      blocking ``urllib`` or synchronous ``httpx`` from inside a coroutine.
    - ``token_issue_secret``: If non-empty, token requests must send ``Authorization: Bearer <secret>`` or
      ``X-Femtobot-Auth: <secret>``.
...
class WebSocketChannel(BaseChannel):
    ...
    async def start(self) -> None:
        from femtobot.utils.logging_bridge import redirect_lib_logging
