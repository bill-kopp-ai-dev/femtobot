...
class OutboundMessage:
    """Message to send to the CLI or another A2A agent.

    ``metadata`` can carry routing (``message_id``, …), trace flags (``_progress``),
    and optional ``OUTBOUND_META_AGENT_UI`` blobs for rich clients.

    TODO(A2A): Prepare this structure to be serialized over HTTP for Docker A2A.
    """
