...
Ported from nanobot (which has the original, well-tested implementation) so the
Femtobot runner can compose its ``messages_for_model`` exactly the way the
upstream project does — including the conditional in-flight overflow
compaction that Femtobot was previously running unconditionally.
"""
...
from femtobot.utils.runtime import ensure_nonempty_tool_result
...
class ContextGovernanceConfig:
    provider: Any  # LLMProvider (forward ref to avoid runtime import)
    model: str
    tools: Any
    workspace: Path | None
...
class ContextGovernor:
    ...
    def compact_inflight_overflow(
        ...
        Key safeguards vs. the unconditional ``_microcompact`` Femtobot had
        before this port (see femtobot/agent/runner.py prior to the fix):

        * Only runs when ``estimate > budget`` (real overflow).
        * Skips messages at indices < ``config.inflight_start_index`` (does
          not touch already-consolidated history).
        * Respects ``compacted_tool_call_ids`` (does not re-compact what was
          already compacted earlier in the turn).
        * Has a hard-overflow fallback that only kicks in when primary
          candidates cannot bring the prompt under budget.
        """
        budget = self.input_budget(config)
        if budget <= 0:
            return messages

        tools = config.tools.get_definitions() if hasattr(config.tools, "get_definitions") else []
