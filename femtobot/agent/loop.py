...
from femtobot.agent import context as agent_context
from femtobot.agent import model_presets as preset_helpers
from femtobot.agent.autocompact import AutoCompact
from femtobot.agent.context import ContextBuilder
from femtobot.agent.hook import AgentHook, CompositeHook
from femtobot.agent.memory import Consolidator
from femtobot.agent.progress_hook import AgentProgressHook
from femtobot.agent.runner import _MAX_INJECTIONS_PER_TURN, AgentRunner, AgentRunSpec
from femtobot.agent.tools.context import RequestContext, bind_request_context, reset_request_context
from femtobot.agent.tools.file_state import FileStateStore, bind_file_states, reset_file_states
from femtobot.agent.tools.message import MessageTool
from femtobot.agent.tools.registry import ToolRegistry
from femtobot.agent.tools.self import MyTool
from femtobot.bus.events import InboundMessage, OutboundMessage
from femtobot.bus.progress import build_bus_progress_callback
...
from femtobot.session import turn_continuation
...
class AgentLoop:
    ...
    def _refresh_provider_snapshot(self) -> None:
        ...
        try:
            ...
            return
        default_selection = snapshot.signature[:2] if snapshot.signature else None
        ...
        if snapshot.signature == self._provider_signature:
            return
        self._default_selection_signature = snapshot.signature[:2] if snapshot.signature else None
    ...
    def set_model_preset(self, name: str | None, *, publish_update: bool = True) -> None:
        ...
        if name not in self.model_presets:
            raise KeyError(f"model_preset {name!r} not found.")
    ...
    def _set_tool_context(
        ...
        from femtobot.agent.tools.context import ContextAware
    ...
    async def _cancel_active_tasks(self, key: str) -> int:
        """Cancel and await all active tasks for *key*.

        Returns the total number of cancelled tasks.
        """
        ...
        try:
            result = await self.runner.run(
                AgentRunSpec(
                    ...
                    # Sustained goals may legitimately exceed FEMTOBOT_LLM_TIMEOUT_S; idle stall
                    # is still capped by FEMTOBOT_STREAM_IDLE_TIMEOUT_S in streaming providers.
    ...
    async def _dispatch(self, msg: InboundMessage) -> None:
        ...
        try:
            async with lock, gate:
                ...
                pending = asyncio.Queue(maxsize=_MAX_PENDING_QUEUE_SIZE)
    ...
    def _assemble_outbound(
        ...
        *,
        ...
        preview = (
            final_content[:_MAX_PREVIEW_LEN] + "..."
            if len(final_content) > _MAX_PREVIEW_LEN
            else final_content
        )
    ...
    async def _state_restore(self, ctx: TurnContext) -> TurnState:
        ...
        preview = (
            msg.content[:_MAX_INBOUND_PREVIEW_LEN] + "..."
