...
from femtobot.agent.tools._constants import (
    SEARCH_DEFAULT_FILE_HEAD_LIMIT,
    SEARCH_DEFAULT_HEAD_LIMIT,
    SEARCH_MAX_FILE_BYTES,
    SEARCH_MAX_RESULT_CHARS,
    SEARCH_TYPE_GLOB_MAP,
)
from femtobot.agent.tools.filesystem import ListDirTool, _FsTool
...
def _matches_type(name: str, file_type: str | None) -> bool:
    ...
    if not lowered:
        return True
    patterns = SEARCH_TYPE_GLOB_MAP.get(lowered, (f"*.{lowered}",))
        ...
        **kwargs: Any,
        ...
        try:
            ...
            if head_limit is not None:
                ...
                limit = SEARCH_DEFAULT_HEAD_LIMIT
            blocks: list[str] = []
            result_chars = 0
            ...
            for file_path in self._iter_files(target):
                ...
                if len(raw) > SEARCH_MAX_FILE_BYTES:
                    skipped_large += 1
                    continue
                if _is_binary(raw):
                    skipped_binary += 1
                ...
                for idx, line in enumerate(lines, start=1):
                    ...
                    if result_chars + extra_sep + len(block) > SEARCH_MAX_RESULT_CHARS:
                        size_truncated = True
                        break
                    blocks.append(block)
