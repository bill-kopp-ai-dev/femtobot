...
class Schema(ABC):
    ...
    Concrete types live in :mod:`femtobot.agent.tools.schema`; all implement
    ...
    def to_json_schema(self) -> dict[str, Any]:
        ...
        if hasattr(self, "_default") and self._default is not None:
            result["default"] = getattr(self, "_default")

        if hasattr(self, "_enum") and self._enum is not None:
            result["enum"] = list(getattr(self, "_enum"))

        if getattr(self, "_nullable", False):
            t = result.get("type")
            if isinstance(t, str):
                result["type"] = [t, "null"]
    ...
    def _base_schema(self) -> dict[str, Any]:
        """Return the base JSON schema fragment for this type."""
        ...
