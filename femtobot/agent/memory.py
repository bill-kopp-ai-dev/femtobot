...
from femtobot.utils.gitstore import GitStore
...
class MemoryStore:
    ...
    # R2 (of the eighth-pass parity review, ``docs/dream_parity_review.md``):
    # the durable memory files whose real working-tree delta grounds
    # Dream commit messages and the cursor-advance gate.  Deliberately
    # excludes ``memory/.dream_cursor`` so that advancing the cursor
    # itself is never mistaken for a productive edit.
    _DREAM_CONTENT_PATHS = ("SOUL.md", "USER.md", "memory/MEMORY.md")
    # R5: per-file cap when embedding current contents into the Dream
    # prompt.  The durable files are tiny in practice (~5 KB total),
    # but a runaway file must not unbounded the prompt.
    _DREAM_FILE_EMBED_CAP = 8000
    # R4: per-session keys / prefixes that are considered "internal"
    # and must be filtered out of the Dream history tail.
    _INTERNAL_HISTORY_SESSION_PREFIXES = ("cron:", "dream:")
    _INTERNAL_HISTORY_SESSION_KEYS = {"heartbeat"}
    # R6: maximum chars of an overridden dream.md prompt that we
    # will pass through.  Mirrors nanobot v0.2.1.
    _DREAM_PROMPT_MAX_CHARS = 32000
    ...
    def __init__(self, workspace: Path, max_history_entries: int = _DEFAULT_MAX_HISTORY):
        ...
        self._dream_prompt_oversize_logged = False  # R6: rate-limit oversize dream prompt
    ...
    # -- R4 (eighth-pass parity review) ------------------------------------
    # Helper that classifies a session key as "internal" (cron, dream,
    # heartbeat).  Used by ``read_recent_history_for_prompt`` so the
    # Dream run never picks up entries generated by other internal
    # sessions.  Without this filter the Dream history tail
    # accidentally picks up cron ticks and heartbeat pings,
    # producing busy-work or no-op Dream runs.
    @classmethod
    ...
    def read_recent_history_for_prompt(
        ...
        *,
        ...
        When ``session_key`` is given, the result is filtered to that
        session.  When ``unified_session`` is true, entries from the
        given session PLUS entries that are *not* internal sessions
        are returned, so a unified Dream turn can see a user's
        history tail without cross-contamination from internal
        ticks.  See ``_is_internal_history_session`` for the
        classification.
        """
    ...
    def build_dream_prompt(self, *, max_entries: int = 20) -> tuple[str, int] | None:
        ...
        Implements R1, R2, R5, and R6 of the eighth-pass parity review
        (``docs/dream_parity_review.md``):

        * **R1** embeds the current contents of the durable memory
          files (SOUL.md, USER.md, memory/MEMORY.md) so the model
          edits the real files rather than a stale mental model —
          eliminating a class of failed/out-of-bounds edits that
          previously produced hallucinated audit records.
        * **R5** truncates each embedded file to
          ``_DREAM_FILE_EMBED_CAP`` (8000) chars with a rate-limited
          warning, so a runaway file cannot unbounded the prompt.
        * **R6** allows a workspace-level override at
          ``workspace/prompts/dream.md`` and truncates it to
          ``_DREAM_PROMPT_MAX_CHARS`` (32000) chars.
        """
        from femtobot.agent.skills import BUILTIN_SKILLS_DIR
        ...
        history_text = "\n".join(
            f"[{e['timestamp']}] {truncate_text(e['content'], 500)}" for e in batch
        )

        # R6: workspace override path
        override_path = self.workspace / "prompts" / "dream.md"
        if override_path.is_file():
            template = override_path.read_text(encoding="utf-8", errors="replace")
            if len(template) > self._DREAM_PROMPT_MAX_CHARS:
                if not self._dream_prompt_oversize_logged:
                    logger.warning(
                        "workspace dream.md is {} chars; truncating to {}",
                        len(template),
                        self._DREAM_PROMPT_MAX_CHARS,
                    )
                    self._dream_prompt_oversize_logged = True
                template = template[: self._DREAM_PROMPT_MAX_CHARS]
        else:
        ...
        # R1 + R5: embed current contents of durable files (truncated per-file)
        ...
        return ("\n\n".join(prompt_parts), batch[-1]["cursor"])

    def _render_current_memory_files(self) -> str:
        """Render current contents of durable memory files for the Dream prompt.

        R1 of the eighth-pass parity review: this gives the model
        the real baseline so edits are bounded by the actual file
        contents, eliminating hallucinated audit records.
        R5 truncates each file to ``_DREAM_FILE_EMBED_CAP`` (8000)
        chars with rate-limited warning.

        Returns "" if no file is present (a fresh workspace).  Missing
        files are silently skipped.
        """
        sections: list[str] = []
        for rel in self._DREAM_CONTENT_PATHS:
            ...
            try:
                ...
                continue
            if len(text) > self._DREAM_FILE_EMBED_CAP:
                if not getattr(self, "_dream_embed_truncated_logged", False):
                    logger.warning(
                        "DREAM embed: {} is {} chars; truncating to {}",
                        rel,
                        len(text),
                        self._DREAM_FILE_EMBED_CAP,
                    )
                    self._dream_embed_truncated_logged = True
                text = text[: self._DREAM_FILE_EMBED_CAP] + "\n...[truncated]"
    ...
    def dream_content_diff(self) -> str:
        """Return the git ground-truth diff of the durable memory files.

        R2 of the eighth-pass parity review: this is the seam
        that closes the "self-report vs ground-truth" gap.  The
        cursor advance gate (``advance_dream_cursor_after_commit``)
        and the commit message builder use this for ground truth
        instead of the LLM's self-report, so a valid no-op Dream
        run (model completes but edits nothing) no longer advances
        the cursor anyway.

        Returns "" when the repo is not initialized, when no
        durable file has changed since HEAD, or when reading the
        repo fails.  Otherwise returns a unified diff body.
        """
        if not self.git.is_initialized():
            return ""
        return self.git.summarize_working_tree(list(self._DREAM_CONTENT_PATHS))

    # R5: rate-limit flag for file embed truncation warnings
    @property
    def _dream_embed_truncated_logged(self) -> bool:
        return getattr(self, "_dream_embed_truncated_logged_attr", False)

    @_dream_embed_truncated_logged.setter
    def _dream_embed_truncated_logged(self, value: bool) -> None:
        self._dream_embed_truncated_logged_attr = value

    def build_dream_tools(self):
        """Build the restricted tool registry used by Dream runs."""
        from femtobot.agent.skills import BUILTIN_SKILLS_DIR
        from femtobot.agent.tools.apply_patch import ApplyPatchTool
        from femtobot.agent.tools.file_state import FileStates
        from femtobot.agent.tools.filesystem import EditFileTool, ReadFileTool, WriteFileTool
    ...
    def build_dream_commit_message(
        prefix: str, resp: object | None, *, diff_body: str = ""
    ) -> str:
        """Build a Dream auto-commit message.

        R2 of the eighth-pass parity review: when *diff_body* is
        given (the ground-truth git diff of the durable memory
        files), the commit message is anchored to the actual file
        delta rather than the LLM's self-report.  When *diff_body*
        is empty, we fall back to the resp summary for the
        one-liner, preserving the legacy behavior.  See
        ``docs/dream_parity_review.md`` for the rationale.
        """
        if diff_body:
            return f"{prefix}\n\n{diff_body.strip()}"
        msg = prefix
