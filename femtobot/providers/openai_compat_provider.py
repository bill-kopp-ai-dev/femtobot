...
from femtobot.providers.openai_responses import (
    consume_sdk_stream,
    convert_messages,
    convert_tools,
...
_DEFAULT_OPENROUTER_HEADERS = {
    "HTTP-Referer": "https://github.com/HKUDS/femtobot",
    "X-OpenRouter-Title": "Femtobot",
...
# Thinking styles mappings for model names
...
def _openai_compat_timeout_s() -> float:
    ...
    return _float_env("FEMTOBOT_OPENAI_COMPAT_TIMEOUT_S", _OPENAI_COMPAT_REQUEST_TIMEOUT_S)
...
def _is_local_endpoint(
    ...
    Matches common private-network patterns in the base URL
    (localhost, 127.x, 192.168.x, 10.x, 172.16-31.x, Docker ``host.docker.internal``).
    """
    if not api_base:
        return False
...
class OpenAICompatProvider(LLMProvider):
    ...
    def _should_use_responses_api(
        ...
        if self._api_type == "chat_completions":
            return False
        if self._spec and self._spec.name not in ("openai",):
            return False
    ...
    def _handle_error(
        ...
        *,
        ...
        if _is_local_endpoint(api_base) and (
            "502" in text or "connection" in text or "refused" in text
        ):
            msg += (
                "\nHint: this is a local model endpoint. Check that the local server is reachable at "
                f"{api_base}, and if you are using a proxy/tunnel, make sure it "
    ...
    async def chat_stream(
        ...
        await self._ensure_client()
        idle_timeout_s = int(os.environ.get("FEMTOBOT_STREAM_IDLE_TIMEOUT_S", "90"))
        try:
            if self._should_use_responses_api(model, reasoning_effort):
                try:
                    body = self._build_responses_body(
