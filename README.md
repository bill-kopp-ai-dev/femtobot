...
|------|---------|-------|-------|-------------|
| **E** | **v0.1.0** (current) | Hardening pós-release | 11 bugfixes E1–E11 | 30 regression |
...
> **v0.1.0 (current):** Lote E is a hardening milestone — no new features, no breaking changes.  Audit ran `ruff` (zero errors), `pytest` (536 passing),  and a deep manual review (race conditions, security, AttributeError crash paths).  The full diff + regression coverage is in [CHANGELOG.md](./CHANGELOG.md).
